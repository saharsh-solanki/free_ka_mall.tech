...
<center><div  class="card text-black  mb-3" style="width: 80%;">
  <div class="card-header"><h3>Wel-Come To Paytm Badshah</h3></div>
</div><hr>
<div  class="card text-black  mb-3" style="width: 80%;">
  <div class="card-header"><h3>Video Result</h3></div>
</div></center>
<hr>
	...
	$getVideo .= '<p><h4>Video Name : -</h4><br>'.$this->get('video_title').'</p></div>';
	if ($this->get('No Stream Available For This video', false) === true) {
		$getVideo .= '<p>No encoded format stream found for this video.</p>
		...
		$getVideo .= '<h2>List of available formats for download</h2><hr>
		<ol class="dl-list">';
		foreach($this->get('streams', []) as $format) {
			if ($format['size'] != 0){
				...
				if ($format['show_direct_url'] === true) {
					$getVideo .= '<a class="btn btn-success btn-download" href="'.$format['direct_url'].'" class="mime"><i class="glyphicon glyphicon-download-alt"></i> Direct</a>';
				}
				...
				$getVideo .= '<div class="label label-warning">SIZE :-'.$format['size'].'</div>
